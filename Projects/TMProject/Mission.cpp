...
int CMission::Read_RMixListFile(const char* filename)
{
    // not used
    return 0;
}

int CMission::Read_NMixListFile(const char* filename)
{
    // not used
    return 0;
}
...
void CMission::ResultItemListSet(unsigned int Head, int X, int Y)
{
    m_NPCHead = Head;
    m_dNPCX = X;
    m_dNPCY = Y;
    ...
    for (int i = 0; i < 24; ++i)
    {
        if (m_dResultIndexList[i])
        {
            ...
            m_pGridResultItem[count]->m_eGridType = TMEGRIDTYPE::GRID_MISSION_RESULT;
            ...
            for (int j = 0; j < 8; ++j)
            {
                ...
                if (stResult_itemList[i].dNeedItemList[j].Access)
                {
                    if (stResult_itemList[i].dNeedItemList[j].Access == 2)
                    {
                        ...
                        if (pFScene != nullptr)
                        {
                            pFScene->SetVisibleMixPanel(pFScene->m_MissionClass.m_pMissionPanel->m_bVisible == 0);
                            pFScene->m_MissionClass.ResultItemListSet(stResult_itemList[i].dNeedItemList[j].dINDEX, 0, 0);
                            pFScene->SetVisibleMixPanel(pFScene->m_MissionClass.m_pMissionPanel->m_bVisible == 0);
                        }
                    }
                    else if (stNeed_itemList[index].stGridItem.sIndex > 0)
                    {
                        ...
                        m_pGridNeedItem[j]->m_eGridType = TMEGRIDTYPE::GRID_MISSION_NEED;
                    }
                }
                else
                {
...
void CMission::ClearResultGridList()
{
    ...
    for (int i = 0; i < 24; ++i)
    {
        m_pGridResultItem[i] = static_cast<SGridControl*>(m_pControlContainer->FindControl(i + G_MISSION_RESULT_1));
...
void CMission::ClearNeedGridList()
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        m_pGridNeedItem[i] = static_cast<SGridControl*>(m_pControlContainer->FindControl(G_MISSION_NEED_1 + i));
    ...
    for (int i = 0; i < 8; ++i)
    {
        auto s_pName = static_cast<SText*>(m_pControlContainer->FindControl(T_MISSION_NAME_1 + i));
        auto s_pVolume = static_cast<SText*>(m_pControlContainer->FindControl(T_MISSION_VOLUME_1 + i));
...
void CMission::SetTextList()
{
    for (int i = 0; i < 8; ++i)
    {
        auto s_pName = static_cast<SText*>(m_pControlContainer->FindControl(i + T_MISSION_NAME_1));
        auto s_pVolume = static_cast<SText*>(m_pControlContainer->FindControl(i + T_MISSION_VOLUME_1));
            ...
            if (m_dNeedRefer[i].bItemListrefer)
            {
                    ...
                    !m_pGridNeedItem[i]->m_pItemList[0])
                {
                    ...
                    for (int j = 0; j < 4; ++j)
                    {
                        ...
                        auto pName = static_cast<SText*>(m_pControlContainer->FindControl(T_MISSION_NAME_1 + j));
                        auto pVolume = static_cast<SText*>(m_pControlContainer->FindControl(T_MISSION_VOLUME_1 + j));
...
void CMission::DoCombine(SMessagePanel* MessagePanel, SGridControl** GridInvList,bool Is2stClass, int level, int Coin)
{
    int count = 0;
...
int CMission::IsSameItem(STRUCT_MYITEM* Item1, STRUCT_ITEM* Item2, short deep)
{
    if (deep > 3)
    {
...
int CMission::IsSameItem(STRUCT_ITEM* Item1, STRUCT_ITEM* Item2, short deep)
{
    if (deep > 3)
    {
        if (Item1->sIndex != Item2->sIndex)
...
int CMission::IsItemOption_Satisfaction(int index, STRUCT_ITEM* item)
{
    auto Return = 0;
    if (index == 15)
    {
...
void CMission::Read_MixListFile()
{
    BASE_ReadMixList();
}

void CMission::BASE_ReadMixList()
{
    FILE* fp{ nullptr };

    fopen_s(&fp, ".\\MissionItens.bin", "rb");

    if (fp == nullptr)
    {
        MessageBoxA(0, "Can't read MissionItens.bin", "ERROR", 0);
        return;
    }
...
void CMission::BASE_WriteMixItemList()
{
    FILE* fp{ nullptr };

    fopen_s(&fp, ".\\MissionItens.bin", "wb");

    if (fp == nullptr)
    {
        MessageBoxA(0, "Can't generate MissionItens.bin file", "ERROR", 0);
        return;
    }
...
int CMission::MYGetItemSanc(STRUCT_MYITEM* item)
{
...
int CMission::HardCode(SMessagePanel* MessagePanel, SGridControl** GridInvList)
{
