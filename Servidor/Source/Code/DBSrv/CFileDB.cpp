...
//extern int Sapphire; //transferido para o basedef
...
#pragma endregion
	...
	case _MSG_SavingQuit:
		{
			...
			pAccountList[Idx].File.ReceivedItem = m->IsNewbieRe == 1 ? true : false;
	...
	case _MSG_AccountSecure:
	{
		...
		if (pAccountList[Idx].File.Info.NumericToken[0] == -1)
		{
			...
			strcpy(sm2.String, "Senha definida com sucesso.");
	...
	case _MSG_DBCreateArchCharacter:
	{
		...
		if (m->isDivina)
		{
			...
			if (SlotIndex != -1)
			{
				...
				pAccountList[Idx].File.affect[Slot][SlotIndex].Value = 0;
...
void CFileDB::DBGetSelChar(STRUCT_SELCHAR *sel, STRUCT_ACCOUNTFILE *file)
{
	...
	{
		memcpy(sel->MobName[i], file->Char[i].MobName, NAME_LENGTH);
