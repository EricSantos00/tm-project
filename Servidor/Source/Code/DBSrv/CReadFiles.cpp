...
const char* const CReadFiles::IMPORT_ITEM_ERROR_PATH = "../../Common/ImporError/%s";
...
const char* const CReadFiles::UPDATE_USER_PATH = "../../Common/update/*.*";
const char* const CReadFiles::UPDATE_USER2_PATH = "../../Common/update/%s";
...
void CReadFiles::WriteRanking()
{
	...
	char classmaster[6][64] = { "SEM CLASSMASTER", "Mortal", "Arch", "Celestial", "Celestial", "Sub Celestial" };
	...
	for (int i = RankPos::FIRST, pos = 1; i > RankPos::LAST; i--)
	{
		if (rankingSystem.grindRanking.getElement(i)->Name[0] != 0 && rankingSystem.grindRanking.getElement(i)->Level < 1000)
		{
			fprintf(fp, "%d,%s,%d,%s,%s,%d,%d \n", pos, rankingSystem.grindRanking.getElement(i)->Name,
				...
				rankingSystem.grindRanking.getElement(i)->Clan);
			pos++;
		}
	}
	fclose(fp);
