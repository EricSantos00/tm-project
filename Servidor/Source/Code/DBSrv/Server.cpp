...
void DrawConfig()
{
	char *SeparatorA = "____________________________________________________________________________________________________________";
	char* SeparatorB = "------------------------------------------------------------------------------------------------------";
	x = 0;
	y = 0;
	...
	for (int i = 0; i < MAX_SERVER; i++)
	{
		...
		if (i < 9)
			TextOutWind(SeparatorB, PRETO);
		
	}
	TextOutWind(SeparatorA, PRETO);
	...
	TextOutWind(SeparatorA, PRETO);
	TextOutWind("Other Status:", VERMELHO);
...
int ReadConfig() {
	...
	int status = nConfig::ReadConfigNews(PATH_SETTINGS, ConfigJson, 0);
	
	if (!status)
	{
		MessageBox(hWndMain, "Erro ao ler config", "FILE ERROR", NULL);
	}
	return status;
...
int WriteConfig() {
	...
	nConfig::WriteConfigNews(PATH_SETTINGS, ConfigJson, 0);
...
#pragma region Load Base Characters
	...
	if (!nConfig::ReadbaseMob("Config/", "baseMob.json", TK))
	{
		...
		MessageBox(hWndMain, "Erro ao ler TK em baseMob.json", "FILE ERROR", NULL);
		return FALSE;
	};

	memmove(&g_pBaseSet[TK], &g_MobBase[TK], sizeof(STRUCT_MOB));
	...
	if (!nConfig::ReadbaseMob("Config/", "baseMob.json", FM))
	{

		MessageBox(hWndMain, "Erro ao ler FM em baseMob.json", "FILE ERROR", NULL);
		return FALSE;
	};

	memmove(&g_pBaseSet[FM], &g_MobBase[FM], sizeof(STRUCT_MOB));
	...
	if (!nConfig::ReadbaseMob("Config/", "baseMob.json", BM))
	{

		MessageBox(hWndMain, "Erro ao ler BM em baseMob.json", "FILE ERROR", NULL);
		return FALSE;
	};

	memmove(&g_pBaseSet[BM], &g_MobBase[BM], sizeof(STRUCT_MOB));
	...
	if (!nConfig::ReadbaseMob("Config/", "baseMob.json", HT))
	{

		MessageBox(hWndMain, "Erro ao ler HT em baseMob.json", "FILE ERROR", NULL);
		return FALSE;
	};

	memmove(&g_pBaseSet[HT], &g_MobBase[HT], sizeof(STRUCT_MOB));


	/* desativado pois ja esta sendo enviado no config
...
int ProcessAdminMessage(int conn, char *msg)
{
	...
	//std = (MSG_STANDARD*)malloc(MAX_SIZE);
