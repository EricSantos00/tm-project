...
void Exec_MSG_Trade(int conn, char *pMsg)
{
	...
	if (OpponentID <= 0 || OpponentID >= MAX_USER)
	{
		...
		SendClientMsg(conn, g_pMessageStringTable[_NN_Wrong_Trade_Packet1]);
		RemoveTrade(conn);
		return;
	}
	...
	else
	{
		if (lastOpponent)
		{
			SendClientMsg(conn, g_pMessageStringTable[_NN_Already_Trading]);
			RemoveTrade(conn);
			return;
		}
	}
	...
	{
		SendClientMsg(conn, g_pMessageStringTable[_NN_Cant_trade_pkmode]);
		SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Cant_trade_pkmode]);
	...
	{
		SendClientMsg(conn, g_pMessageStringTable[_NN_Deny_Whisper]);
		SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Deny_Whisper]);
	...
	for (int i = 0; i < MAX_TRADE; i++)
	{
		...
		{
			SendClientMsg(conn, g_pMessageStringTable[_NN_Only_With_Guild_Master]);
			SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Only_With_Guild_Master]);
		...
		if (m->Item[i].sIndex && BASE_GetItemAbility(&m->Item[i], EF_NOTRADE) != 0)
		{
			...
			SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Cant_MoveItem]);
			RemoveTrade(conn);
		...
		if (pUser[OpponentID].Trade.OpponentID == conn)
		{
			...
			if (m->MyCheck == 1)
			{
				...
				if (pUser[conn].Trade.TradeMoney > pMob[conn].MOB.Coin)
				{
					...
					SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Opponent_Havent_Money]);
					return;
				}

				if (pUser[OpponentID].Trade.TradeMoney > pMob[OpponentID].MOB.Coin)
				{
					SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Havent_Money_So_Much]);
					SendClientMsg(conn, g_pMessageStringTable[_NN_Opponent_Havent_Money]);
					return;
				}
				...
				{
					SendClientMsg(conn, g_pMessageStringTable[_NN_Each_Of_You_Havent_Space]);
					SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Each_Of_You_Havent_Space]);
					return;
				}
				...
				if (mytrade == 0)
				{
					...
					SendClientMsg(OpponentID, g_pMessageStringTable[_NN_Opponent_Have_No_Space_To]);
					return;
				}

				if (opptrade == 0)
				{
					SendClientMsg(conn, g_pMessageStringTable[_NN_Opponent_Have_No_Space_To]);
					SendClientMsg(OpponentID, g_pMessageStringTable[_NN_You_Have_No_Space_To_Trade]);
					return;
				}
	...
	MyLog(LogType::Itens, pMob[OpponentID].MOB.MobName, temp, 0, pUser[OpponentID].IP);
