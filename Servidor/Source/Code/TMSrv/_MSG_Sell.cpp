...
void Exec_MSG_Sell(int conn, char *pMsg)
{
	...
	{
		SendClientMsg(conn, g_pMessageStringTable[_NN_Only_Merchant_Buy_Yours]);
		return;
	}
...
LABEL_S1:
	...
	if (Vol == 1)
	{
		SendClientMsg(conn, g_pMessageStringTable[_NN_Cant_Sell_Potion]);
		return;
	}
	...
	if (CityTax >= 0 && CityTax <= 30)
	{
		...
		if (sPrice >= 0)
		{
			if (GetRecycleBin(conn, Item, sPrice) == FALSE) return;
		...
		LABEL_S2:
			...
			sprintf(temp, "Adquiriu %d de gold", sPrice);
