...
void CMob::GetCurrentScore(int idx)
{
	...
	else if (idx >= MAX_USER)
	{
		...
		if (geneidx > 0 && geneidx < MAX_NPCGENERATOR)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				if (MOB.Resist[i] > 100) { MOB.Resist[i] = 50; }
			}
		}
...
// Verifica se o usuário está afetado com a magia (Cancelamento / Foema)
	for (int i = 0; i < MAX_FORCE_TABLE; i++)
	{
		CheckSpellState[i] = false;
	}

	for (int i = 0; i < MAX_AFFECT; i++)
	{
		if (Affect[i].Type == FALSE) continue;

		if (Affect[i].Type == CANCEL_SPEEL_INDEX)
		{
			CheckSpellState[CH_SPEEL_CANCELATION] = true;
		}
	}
	...
	//Fada Verde
	if(MOB.Equip[13].sIndex == 3903 || MOB.Equip[13].sIndex == 3906 || MOB.Equip[13].sIndex == 3911 || MOB.Equip[13].sIndex == 3912)
		ExpBonus += 16;
